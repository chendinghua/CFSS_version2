...
public class MyFunc {
    ...
    //-------------------------------------------------------
	public String ByteArrToHex(byte[] inBytArr)//�ֽ�����תתhex�ַ���
	{
		StringBuilder strBuilder=new StringBuilder(inBytArr.length);
		int j=inBytArr.length;
		String sTemp;
		for (int i = 0; i < j; i++)
		{
			sTemp = Integer.toHexString(0xFF & inBytArr[i]);
			if(sTemp.length()<2)
				strBuilder.append(0);
			strBuilder.append(sTemp.toUpperCase());
		}
